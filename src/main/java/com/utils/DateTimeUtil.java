 ...
 *  @author  YangWenjun
 *  @date   2018年10月4日
 *  @description         日期格式转换   ***银行系统中节假日的维护***顺延
 *
 *                                 1.是什么
 *                                 2.为什么
 *                                 3.如何做
 *                                          1.涉及到的类  String  - java.util.Date - java.util.Calendar - java.text.SimpleDateFormat - java.sql.Timestamp - java.text.ParsePosition - Long
 *                                          2.型字段设计，应该如何选择？ http://windshome.iteye.com/blog/1871058
 *                                          3.如下代码分析
 *                                 4.对比 原理  改进
 *  @todo  TODO
 *
...
/**
 * 以下内容是
 *                  1.bbsp中日期维护方法 - 手工维护 - 代码在哪里实现？？？
 *                  2.提供哪些可能出现的转化  date-date? date-str-date来实现吗？还是可以
 */
    ...
    public class DateTimeUtil{
        ...
        public static void main(String [] args){
            try {
                //MainClass dateUtil = new MainClass();
                System.out.println();
...
/**
 * bbsp dateTimeUitl
 * 1.时间分为两类  1是格式 2是calendar 获取年/月/ 几天后 / 日历 两天之间时间差（笨办法是获取最基本值，除以小时*分*秒.....）
 * 提供基础的:public static String  formatDate(Date date , String pattern){
 *   //对参数进行检测
 *     if(date == null)  return null ;
 *     if(pattern == null)   String pattern = "yyyy-MM-dd HH:mm:ss SSS" ;
 *     SimpleDateFormat sdf = new SimpleDateFormat(pattern)；
 *     sdf.format(date)
 * }
 * 也可以提供get_YYYYMMDD_DATE为命名来获取特定格式的时间格式string .
 *            public static String  get_YYYYMMDD_DATE(){
 *                  return new SimpleDateFormate("yyyyMMdd").format(new Date());
 *
 *            }
 * 2.public static Date addDate(Date date,int year,int month,int day,int hour,int minute,int second,int millisecond){
 Calendar c = Calendar.getInstance();
 c.setTime(date);
 c.add(Calendar.YEAR, year);//加减年数

 3.sql.date / util.date
 前者用于插入数据库的值   通过new java.sql.Date(new Calendar().getTime().getTIme())

 4.获取月份最后一天 / 是否为某月最后一天/两日期时间间隔-算前不算后.......DateTimeUtil
 5.TimeZone时区
 */
