       ...
       缓存的设计可复杂也可简单，但是需要考虑的点却很多。
            ...
            1.缓存对象
                ...
                哪些数据应该放到缓存中？依据是什么？缓存数据是采用主动刷新还是过期自动失效？（实效性较低，比如活动．．．可以自动失效．定时刷新、仿真参数由于使用主动刷新）
                    1.字典表、配置类的数据   2.明显是热点数据的数据 菜单、经常查询的业务数据，比对手授信信息-风险、额度列表、到期票据、记账票据...分行..<--跑批后更新
                    3.量不大接口，但处理逻辑复杂。只与日终批点有关 比如到期扣款、提示付款、批量释放...
                    4.首页
                (区别于电商缓存 借助@Cacheable根据用户入参进行缓存，访问量大)
                    参考自：https://blog.csdn.net/hollis_chuang/article/details/105154086

            2.缓存的类型、大小 、缓存对象粒度、结构、对象序列化

                由于大部分的web应用都会尽量做到‘ 无状态 ’，以方便线性扩容
                一般来说，你可以采用对象属性在内存中的存储长度简单加和的方法来计算单个对象的体积，再乘以缓存对象的数量和预期增长
                （当然，这里边有一个热点数据的问题，这里就不细讨论了），大概得出需要使用的缓存空间

                大型web系统数据缓存设计:https://data.qq.com/article?id=2879
            ...
            3.缓存一致性 vs 分布式一致性
                缓存一致性是必须要考虑的点。很多缓存设计都忘记了这一点，导致在使用缓存的时候经常返回过期数据。数据在更新的时候，缓存可以采用如下几个策略
                    ...
                    2.更新数据时，先更新缓存，再更新数据库或者先更新缓存之后异步刷新到数据库 vs 先更新数据库，在更新缓存；
                      参考： https://blog.csdn.net/qq_33999844/article/details/81531461
