 ...
 *      0.宗旨：一定要在异常出现后，让别人知道，想要别人不知道，就必须自己解决，而不是假装没有
        ...
        0.什么算异常？ - 考虑完全
                1.代码本身异常  受检/不受检  -- 二者都是runtimeexception 前者会造成 ' 接口 ' 有抛出异常 - 影响接口定义 ，后者无
                2. response .code != 200
                3. 业务级别异常 比如 reponse.content.bizcode != success 
                4. 特殊异常，有的接口做了降级，返回 reponse.content.bizData = [] 
 ...
 *      3.捕获异常时机
 *              如果你知道如何处理，比如下面的记录，舍去，降级，转换，补偿，重试，那么给出捕获后给出方案。   <<<<<<<<<<<<<<<<<<<================
 ...
 *      1.Controller 层负责信息收集、参数校验、转换服务层处理的数据适配前端，轻业务逻辑；
     ...
     2.异常传递信息vs返回值VS父类msg进行判断。commonvalidator。了解要的点     <<< ==== 统一封装返回
