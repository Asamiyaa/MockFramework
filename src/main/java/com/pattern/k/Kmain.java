 ...
 * 1.幂等vs事务vs线程 -----vs一致性
   ...
   2.常用幂等的操作 给行为打标而不是数据 - 思考模拟器实现注意点  ---同样可以通过切面 aop实现，只是实现方式
         1.全局统一ID请求。前台 token   token做认证保存session信息。和防重区分开。防重要新建token
             token。范围广但多加了一步操作思路是对的，但让客户端先调获取token其实属于代码入侵，而且增加了网络请求。最好结合业务特点来根据当前用户+当前业务数据来做唯一标识来校验更合适。
         2.加锁。乐观。悲观。分布式锁。查询也可以加锁
         3.状态机 消息
         4.幂等表 主键 给行为加标识而不是数据。因为数据的主键可以多次操作
         5.缓存锁。获取锁才能操作。和token结合。获取操作真正出路业务时。插入token是在提交时--session
         6.主键。要求主键有业务含义插入时将幂等放到数据库
